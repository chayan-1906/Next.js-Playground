...
async function CatchAllPage({params}: { params: { slug: string[] }; }) {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <CatchAllWrapper params={params}/>
        </Suspense>
    );
}

async function CatchAllWrapper({params}: { params: { slug: string[] }; }) {
                ...
                <div className={'bg-gray-50 dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800'}>
                    ...
                    <div className={'p-6'}>
                        {slug ? (
                            ...
                            <p className={'text-gray-500 dark:text-gray-400'}>No slug segments were provided</p>
                        )}
                    </div>
                    <div className={'p-4 bg-gray-100 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-800 rounded-b-xl'}>
                        <p className={'text-sm text-gray-600 dark:text-gray-400 font-mono text-center'}>
                            Route: <code>/catch-optional-all-demo/catch-all/{slug.join('/')}</code>
                        </p>
                    </div>
                </div>
            </div>
