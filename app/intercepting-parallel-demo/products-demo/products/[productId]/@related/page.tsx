...
async function RelatedSlot({params}: { params: Promise<{ productId: string }> }) {
    return (
        <Suspense fallback={<div>Loading related products...</div>}>
            <RelatedSlotWrapper params={params}/>
        </Suspense>
    )
}

async function RelatedSlotWrapper({params}: { params: Promise<{ productId: string }> }) {
            ...
            <div className={'grid grid-cols-1 sm:grid-cols-3 gap-6'}>
                {relatedProducts.map((product: Product) => {
                    ...
                    return (
                        <Link
                            key={_id}
                            href={`/intercepting-parallel-demo/products-demo/products/${_id}`}
                            className={'group'}
                        >
                            <div
